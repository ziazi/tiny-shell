...
void eval(char *cmdline) 
{
    ...
    sigset_t blocked ;
    sigemptyset(&blocked);
    //line_buff if for security
    strcpy(line_buf,cmdline);
    int bg=  parseline(line_buf,argv);
    //now argv contains the command
    if(*argv==NULL){
        return;
    }
    if(builtin_cmd(argv)){
...
{
    if(argv[1]==NULL){
        printf("command %s requires an id \n",argv[0]);
        return;
    }
    ...
    if(isdigit(argv[1][0])) { 
     ...
     kill(-job->pid, SIGCONT); // send continuation to the process group pid that are running the job
...
void sigchld_handler(int sig) 
{
 ...
 int childstatus;
 pid_t cid;
 uint32_t flags = WNOHANG | WUNTRACED; // WNOHANG: the handler should not wait, 
                                        //WUNTRACED: a child terminated (may have exited we should check this in the while)
                                        //            or stopped (we should checkt hat and not delete stopped childs)
 ...
 */
void signabort_handler(int sig){
